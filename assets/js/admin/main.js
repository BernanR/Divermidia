...
function captha() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      html += '<div class="mt-3">';
      html +=
        '<span onclick="captha()" class="captha-button" style="cursor:pointer;">';
      html +=
      ...
      html +=
        '<input style="font-size: 12px;" id="captha_input"  name="captha_input" id="captha_input" maxlength="15"  type="text" placeholder="Digite o Texto" class="form-control captha_input">';
      html += "</div>";
      html += "</div>";
      //html += "</div>";
      setTimeout(function () {
        $.ajax({
          type: "GET",
          url: base_url + "captha-remove?file=" + data.filename,
        });
      }, 6000);
